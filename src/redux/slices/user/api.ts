...
const handleSetResetPasswordLink = async (email: string, resetPasswordLink: string, thunkAPI: any) => {
  ...
  try {
    const response = await axios.post('http://localhost:8080/send-reset-password-link', { to: email, link: resetPasswordLink });
    return response.data;
  } catch (error: any) {
  ...
  async (data, thunkAPI) => {
    try {
      ...
      const response = await axios({
        method: 'post',
        url: 'http://localhost:8080/export-files',
        data: {
...
export const requestGenerateAndSendOTP = createAsyncThunk<any, OnRequestGenerateAndSendOTP>(
    ...
    async (data, thunkAPI) => {
      try {
        ...
        const response = await axios.post('http://localhost:8080/generate-send-otp', { email: email, nickName: nickName });
        return response;
      } catch (error: any) {
...
export const requestOTPVerification = createAsyncThunk<any, OnRequestOTPVerification>(
  ...
  async (data, thunkAPI) => {
    try {
      ...
      const response = await axios.post('http://localhost:8080/verify-otp', { email: email, otp: otp });
      return response;
    } catch (error: any) {
...
export const requestPlanSubscription = createAsyncThunk<any, OnRequestPlanSubscription>(
  ...
  async (data, thunkAPI) => {
    try {
      const { plan } = data;
      const response = await axios.post('http://localhost:8080/subscribe', { plan });      
      return response.data;
    } catch (error: any) {
...
export const requestPaymentVerification = createAsyncThunk<any, OnRequestPaymentVerification>(
  ...
  async (data, thunkAPI) => {
    try {
      ...
      const verificationResponse = await axios.post('http://localhost:8080/verify-payment', paymentResponse);
      ...
      if (verificationResponse.status === 200) {
          ...
          const sendInvoiceResponse = await axios.post('http://localhost:8080/send-subscription-invoice', {
