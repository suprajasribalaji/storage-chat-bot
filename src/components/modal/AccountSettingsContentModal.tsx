...
const AccountSettingsContentModal = (props: AccountSettingsContentModalProps) => {
    ...
    const fetchDataOfCurrentUser = async (user_id: string | undefined) => {
        try {
            const userQuery = query(collection(document, "Files"), where("user_id", "==", user_id));
            const querySnapshot = await getDocs(userQuery);
    ...
    //     for (let index = 0; index < fileDownloadURL.length; index++) {
    //         const fileURL = fileDownloadURL[index];
    ...
    return (
        <AccountSettingsContent>
            ...
            {/* <SettingsItem>
                ...
                <StyledEnableExportButton onClick={handleExportButton}>Export</StyledEnableExportButton>
            </SettingsItem> */}
            <SettingsItem>
